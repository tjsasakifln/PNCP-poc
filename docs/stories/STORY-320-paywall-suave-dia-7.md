...
- [x] **AC1:** Criar funcao `get_trial_phase(user)` em `quota.py`:
...
- [x] **AC2:** Endpoint `GET /v1/trial-status` inclui campo `trial_phase` no response
- [x] **AC3:** `POST /buscar` retorna campo `paywall_applied: bool` quando resultados sao truncados
...
- [x] **AC4:** `GET /v1/export/excel` em `limited_access`: gera Excel com 10 resultados + sheet extra "Desbloqueie {N-10} resultados adicionais com SmartLic Pro"
- [x] **AC5:** Feature flag `TRIAL_PAYWALL_ENABLED` (default: true) para desativar rapidamente

### Frontend — UI de Paywall

- [x] **AC6:** Criar `frontend/components/billing/TrialPaywall.tsx`:
...
- [x] **AC8:** Em `SearchResults.tsx` — download Excel:
...
- [x] **AC13:** Testes backend: `get_trial_phase()` retorna fase correta por dia
- [x] **AC14:** Testes backend: busca trunca resultados em `limited_access`
- [x] **AC15:** Testes frontend: TrialPaywall renderiza blur em dia 8+
- [x] **AC16:** Testes frontend: nao renderiza paywall em dias 1-7
- [x] **AC17:** Testes frontend: nao renderiza paywall para usuarios pagos
- [x] **AC18:** Zero regressions
