...
- [ ] AC1: O navigation guard só deve ativar durante uma busca em andamento (`search.loading === true`).
- [ ] AC2: Após os resultados aparecerem, o guard deve se desativar automaticamente após 30 segundos.
- [ ] AC3: Clicar em links internos do SmartLic (Pipeline, Dashboard, Histórico, Conta) nunca deve exibir o dialog de confirmação.
- [ ] AC4: O `beforeunload` (fechar aba/reload) pode permanecer ativo enquanto `loading === true` (protege contra perda de busca em andamento).
- [ ] AC5: Remover a dependência de `hasDownloaded` como único mecanismo de desativação.
...
## Testes Necessários
- [ ] Teste que guard está ativo durante `loading=true`.
- [ ] Teste que guard desativa 30s após `loading` mudar para `false`.
- [ ] Teste que links internos (mesmo origin) não acionam dialog.
- [ ] Teste que `beforeunload` ativa apenas durante loading.
- [ ] Teste que guard NÃO ativa quando resultados estão exibidos mas busca terminou há mais de 30s.
