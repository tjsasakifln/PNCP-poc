...
- [ ] **DB-M02**: Tighten `plan_type` CHECK constraint -- remove legacy values (`free`, `avulso`, `pack`, `monthly`, `annual`)
...
- [ ] **DB-M03 + DB-M05**: Fix `updated_at` migration gap for `user_subscriptions` + make Stripe price IDs environment-aware
...
- [ ] **DB-H05**: Add explicit INSERT policy on `profiles` table
...
- [ ] **CROSS-M01**: Establish single source of truth for plan data across database, backend, and frontend
...
- [ ] **FE-H05**: Extract `UF_NAMES` mapping to shared module
...
- [ ] **FE-NEW-04**: Add page transition indicators (NProgress or similar)
...
- [ ] **FE-M02 + FE-M05**: Expand feature flags system + automate SETORES sync
...
- [ ] **SYS-M02**: Fix token cache hash mechanism -- replace `hash()` with `hashlib.sha256()` for deterministic, collision-resistant caching
...
- [ ] **SYS-M03**: Add max size limit to in-memory rate limiter
...
- [ ] **SYS-M04**: Move plan capabilities from Python code to database
...
- [ ] **SYS-M06**: Replace all `datetime.utcnow()` with `datetime.now(timezone.utc)` across codebase
...
- [ ] **SYS-H03**: Establish tracked migration workflow in repository
...
- [ ] **DB-L02 + DB-L03**: Implement retention cleanup via pg_cron
...
- [ ] **NEW-DB-05**: Document `plan_id` FK ON DELETE behavior as intentional RESTRICT
...
- [ ] **SYS-L01**: Add OpenAPI schema validation in tests
...
- [ ] **SYS-L03**: Replace inline CSS in layout.tsx with CSS custom properties
...
- [ ] **FE-M04**: Extract `STOPWORDS_PT` from buscar/page.tsx to shared module
...
- [ ] **FE-M06 + FE-M07**: Delete stale files (`dashboard-old.tsx`, `landing-layout-backup.tsx`)
...
- [ ] **FE-M09**: Replace deprecated `performance.timing` with `performance.getEntriesByType('navigation')`
...
- [ ] **FE-L02**: Fix `useEffect` with serialized Set dependency
...
- [ ] **FE-L04**: Delete unused barrel file `components/filters/index.ts`
  - Files: `frontend/components/filters/index.ts` (delete)
  - Validation: File removed. No imports reference it.
  - Effort: 0.5h

- [ ] **FE-L05**: Add `robots.txt` and `sitemap.xml`
...
- [ ] **FE-L06**: Configure OpenGraph images for social sharing
...
- [ ] **FE-NEW-05**: Add breadcrumbs to protected pages
...
- [ ] **FE-NEW-08**: Cache sector list client-side (memory or localStorage with short TTL)
...
- [ ] **FE-NEW-09**: Add `/mensagens` and `/planos/obrigado` to middleware PROTECTED_ROUTES list
