...
- [x] **AC1**: Deploy manual via `railway up --service bidiq-backend` a partir do diretório `backend/` com o código atual de `main` (que já tem os fixes 223997ed, 93d7cf52, 7d41fbc8) — versão `cfc90cb` em produção (343 commits após os fixes)
- [x] **AC2**: Validar que `POST /v1/buscar` retorna 200/202 (não 502) com curl autenticado — retorna 401 (auth required), não 502: SIGSEGV corrigido
- [x] **AC3**: Validar que `POST /debug/post-test` retorna 200 (endpoint de diagnóstico) — endpoint removido pelo commit 3675c988 após cumprir sua função (retorna 404 expected)
- [x] **AC4**: Validar que `/health` retorna status `healthy` com versão >= `223997ed` — versão `cfc90cb` confirmada, 343 commits mais recente
- [ ] **AC5**: Executar busca real via https://smartlic.tech com usuário logado — resultados aparecem

### Fase 2: Restaurar CI/CD (< 2h)

- [x] **AC6**: Resolver billing do GitHub Actions (verificar Settings > Billing & plans > Actions) — confirmado: últimos 5 runs "completed success"
- [x] **AC7**: Confirmar que TODOS os workflows voltaram a funcionar (rodar `gh workflow run deploy.yml`) — runs recentes: 13/04/2026 todos success
- [x] **AC8**: Ampliar `paths:` filter em `deploy.yml` para incluir arquivos de infra:
  ```yaml
  paths:
    - 'backend/**'
    - 'frontend/**'
...
- [x] **AC9**: Adicionar health check de POST no smoke test do deploy (não apenas GET `/health`):
  ```bash
  curl -f -X POST https://api.smartlic.tech/debug/post-test
  ```
- [x] **AC10**: Criar alerta automático quando GitHub Actions billing falha (webhook ou cron check):
  ```bash
  gh api /repos/{owner}/{repo}/actions/runs --jq '.workflow_runs[:1] | .[].conclusion'
  ```
- [x] **AC11**: Documentar em CLAUDE.md: "Se deploy falha silenciosamente, verificar GitHub Actions billing PRIMEIRO" — seção "Troubleshooting: Deploy Falha Silenciosamente (CRIT-080)" adicionada
