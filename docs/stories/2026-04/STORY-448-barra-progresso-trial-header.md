...
- [x] Componente `TrialProgressBar` renderizado abaixo da navbar principal
- [x] Visível em TODAS as páginas autenticadas exceto: `/login`, `/signup`, `/onboarding`, `/auth/*`
- [x] Visível APENAS para usuários com `plan_type === "free_trial"` E trial não expirado

### AC2: Conteúdo dinâmico da barra
- [x] Texto: "Dia {X} de 14 — Você já fez {Y} buscas e encontrou {Z} editais."
- [x] Botão/link à direita: "Ver Planos →" com href="/planos"
- [x] {X}: calculado de `trial_expires_at` (disponível no contexto de auth)
- [x] {Y} e {Z}: buscados do analytics summary endpoint existente (`GET /api/analytics?endpoint=summary` ou similar)
- [x] Se Y/Z não disponíveis (erro de fetch): exibir apenas "Dia {X} de 14 — Veja os planos antes que seu trial expire."

### AC3: Cor muda com urgência do trial
- [x] Dias 1-7: fundo verde claro, texto verde escuro (`bg-green-50 text-green-800`)
- [x] Dias 8-11: fundo amarelo claro, texto amarelo escuro (`bg-yellow-50 text-yellow-800`)
- [x] Dias 12-14: fundo vermelho claro, texto vermelho escuro (`bg-red-50 text-red-800`)

### AC4: CTA de conversão
- [x] Link "Ver Planos →" usa `<Link href="/planos">` (não abre nova aba)
- [x] CTA visualmente em destaque (bold ou cor de acento) dentro da barra

### AC5: Não aparece para pagantes/expirados
- [x] Usuários com plano pago (`smartlic_pro`, `consultoria`, etc.) NÃO veem a barra
- [x] Usuários com trial expirado NÃO veem a barra (eles veem o hard block em /planos)
- [x] Usuários não autenticados NÃO veem a barra

### AC6: Exclusão de páginas
- [x] Barra NÃO aparece em: `/login`, `/signup`, `/onboarding`, `/auth/callback`, `/recuperar-senha`, `/redefinir-senha`
- [x] Barra NÃO aparece em páginas de marketing públicas: `/`, `/pricing`, `/features`, `/termos`, `/privacidade`

### AC7: Tracking Mixpanel
- [x] Click em "Ver Planos →" dispara: `trial_progress_bar_cta_clicked` com propriedade `trial_day: number`

### AC8: Testes
- [x] Teste: trial dia 5 → barra verde visível com "Dia 5 de 14"
- [x] Teste: trial dia 10 → barra amarela
- [x] Teste: trial dia 13 → barra vermelha
- [x] Teste: pagante → barra não renderizada
- [x] Teste: trial expirado → barra não renderizada
- [x] Teste: página /login → barra não renderizada
...
- [x] `frontend/components/TrialProgressBar.tsx` — AC1, AC2, AC3, AC4, AC5: novo componente
- [x] `frontend/app/layout.tsx` — AC1, AC6: integrar TrialProgressBar condicionalmente
- [x] `frontend/__tests__/TrialProgressBar.test.tsx` — AC8: testes unitários
