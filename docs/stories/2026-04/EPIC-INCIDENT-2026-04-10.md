...
- [ ] Todas as 17 stories em status `Done` _(412-423 em InReview aguardando janela Sentry 48h. 425-429 em InReview — implementadas em YOLO sprint 2026-04-11.)_
...
| 2026-04-11 | @dev (YOLO sprint 425-429) | **5 stories implementadas em paralelo:** STORY-425 (P0 schema drift `data_publicacao_pncp` → fix Opção B: `data_publicacao` real, bundled com STORY-426 timeout guard). STORY-426 (P1 timeout guard: `asyncio.wait_for(to_thread, 6s)` + degraded 200 + `SUPABASE_QUERY_TIMEOUT_TOTAL` metric). STORY-427 (P1 deque race: snapshot `list(self._window)` em `_record_failure` + `RuntimeError` wrapped como `CircuitBreakerOpenError` em `sb_execute` + `SUPABASE_CB_INTERNAL_ERRORS` metric). STORY-428 (P2 auditoria rotas Next.js: nenhum conflito `[setor]`/`[cnpj]` encontrado — story encerrada). STORY-429 (P2 error code case: 9 call sites corrigidos para `ErrorCode.XXX.value` em execute.py/validate.py/routes/search.py). 32 novos testes passando. Zero regressões em `test_supabase_circuit_breaker.py` (57 pass, 1 pre-existing fail). Todas 5 stories → InReview. |
| 2026-04-11 | @dev (YOLO sprint ACs pendentes) | **4 ACs genuinamente pendentes implementados em paralelo:** STORY-416 AC4 (`routes/analytics.py` × 3 endpoints + `sessions.py` + `pipeline.py` → HTTP 200 + `X-Cache-Status: stale-due-to-cb-open` quando CB open, fallback com dados zerados, 6 novos testes em `test_story416_ac4_graceful_degradation.py`). STORY-417 AC2 (`empresa_publica._fetch_contratos_local` trocado para `sb_execute(..., category="read")` + catch `CircuitBreakerOpenError` → PT fallback). STORY-417 AC3 (partial response HTTP 200 + `partial: bool` em `PerfilB2GResponse`; HTTP 206 descartado — risco fetch clients; 4 novos testes em `test_story417_brasilapi_cb.py`). STORY-418 AC5 (`trial_email_sequence.py` trocado check-then-insert por upsert `ON CONFLICT DO NOTHING` via `ignore_duplicates=True`). STORY-419 AC3 (`ValorFilter.tsx` clamp a `VALOR_MAX_SYSTEM_LIMIT=999_999_999_999_999` com mensagem amigável + estado `maxClamped`; 7 testes em `ValorFilter.test.tsx`). STORY-418 AC3 e STORY-427 AC3 confirmados já implementados (apenas checkboxes atualizados). Ruff/mypy/tsc limpos. 47 novos testes passando. |
...
| 2026-04-22 | @po (Sarah) | `*validate-story-draft` (revalidação) verdict **GO** (score 10/10). Transição **Draft → Ready** aplicada. Ressalva de estado zombie: 17 stories children já estão em InReview (12 do grupo 412-423 + 5 do grupo 425-429 pós-varredura 2026-04-11); o header do epic estava preso em Draft apesar do trabalho avançado. A nota do Change Log de 2026-04-10 "@po — Status Draft → Ready para todas" refere-se às stories filhas, não ao epic. Transição do epic para Ready é pré-requisito formal para @qa encerrar Done do epic após conclusão da janela de observação Sentry (48h pós-deploy). Todas as 6 categorias PASS: goal P0 claro, 17 stories children tabeladas com status/priority/owner, 8 decisões @pm documentadas, DoD com 6 critérios (alguns já [x]), KPIs quantificados (error rate, slow requests, issues Sentry). Relatório completo: `docs/po-review-2026-04-22.md`. |
