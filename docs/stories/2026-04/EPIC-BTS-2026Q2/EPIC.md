...
- **2026-04-20** — @dev: **Wave 3 code landed; CRIT-054 merged; BTS-009 in review**. STORY-BTS-009 (25/25 PASS em worktree, 5 extras triaged vs 20 esperados) via PR #410. STORY-CRIT-054 (PR #407 merged `2ff704a4`) restaurou `elif status_inferido in ("desconhecido", "todos")` pass-through em `backend/filter/pipeline.py`. Autouse fixture `_reset_startup_shutting_down_state` experimentada em PR #411 mas revertida em `532eafcf` (net-negative em batch vs isolated). Baseline-zero via PR #411 (`5994dedc`): 12 fixes validados + 20 xfails documentados para pollution batch-only. **Após #407 + #411 merged, main CI re-run revelou nova baseline de ~150 falhas em ~8-10 drift clusters** — totalmente separadas do escopo BTS-009. Essas drifts viram escopo de **STORY-BTS-011** (próxima sweep). PR #424 já ligou 4 drift clusters / 15 falhas. **Epic fica 10/11 Done** até PR #410 mergear; fechamento formal aguarda BTS-011 destravar main CI. Session handoff: `docs/sessions/2026-04/2026-04-20-majestic-valiant-handoff.md`.
- **2026-04-20 (late)** — @dev + @devops: **STORY-BTS-011 MERGED via PR #426 (`431b6154`)**, fechando 7 clusters de drift pós-#407 — 37+ testes corrigidos (3 corrections + 4 xfails documentados para BTS-012 residual). Main CI **Backend Tests (PR Gate) voltou a VERDE**: últimos 3 runs em main consecutivos = SUCCESS (run 24683564675 após merge PR #423 CONV-003 AC2, run 24683629411 após merge PR #427 squads, run 24683689663 após merge docs BTS-011 closure). **Epic agora só precisa PR #410 (BTS-009) rebasear em main fresca e mergear** para fechar formal. Main CI gate essentially restored — 3 consecutive green runs já é evidência sólida da estabilização estrutural.
