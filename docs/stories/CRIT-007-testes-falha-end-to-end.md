...
### Infrastructure
- [x] AC1: Criar diretorio `backend/tests/integration/` com `conftest.py` contendo fixtures compartilhadas
- [x] AC2: Fixtures devem incluir: mock Supabase, mock PNCP responses, mock Redis, test FastAPI client
- [x] AC3: Integration tests devem usar `TestClient` do FastAPI (nao mocks de pipeline stages)
- [x] AC4: Habilitar job `integration-tests` no CI (`tests.yml`) — remover stub, rodar pytest real
- [x] AC5: Integration tests devem ter marker `@pytest.mark.integration` para execucao separada

### P0 Failure Cascade Tests
- [x] AC6: **test_full_pipeline_cascade.py** — Test SearchPipeline com todos os stages reais (nao mocked):
...
- [x] AC7: **test_queue_inline_fallback.py** — Test que quando `is_queue_available()` retorna False:
...
- [x] AC8: **test_supabase_total_outage.py** — Test com ALL Supabase operations failing:
...
- [x] AC9: **test_absolute_worst_case.py** — All sources + all caches fail:
  - PNCP returns 500
  - PCP returns 500
...
- [x] AC10: **test_post_independent_of_sse.py** — Test POST /buscar without SSE:
...
- [x] AC11: **test_frontend_504_timeout.py** — Frontend proxy 504:
...
- [x] AC12: **test_queue_worker_fail_inline.py** — Queue dispatch + worker fail:
...
### P1 Canary Tests
- [x] AC13: **test_schema_canary.py** — Schema compatibility:
...
- [x] AC14: **test_pncp_api_canary.py** — PNCP API contract:
...
- [x] AC15: **test_concurrent_searches.py** — Concurrent search isolation:
...
### Frontend Tests
- [x] AC16: Tirar `useSearch.test.ts` da quarentena e corrigir:
...
- [x] AC17: **useSearch-failures.test.ts** — New test file for failure scenarios:
...
- [x] AC18: **Playwright E2E: failure-scenarios.spec.ts**:
...
### Documentation
- [x] AC19: Cada teste P0 deve ter docstring explicando o cenario de producao que ele previne
- [x] AC20: README em `backend/tests/integration/README.md` com instrucoes de execucao
...
- [x] `pytest backend/tests/integration/ -v` — todos passam (43/43)
- [x] `npm test -- --testPathPattern="useSearch"` — passa (fora da quarentena) (40/40)
- [x] `npm test -- --testPathPattern="useSearch-failures"` — passa
- [x] CI integration-tests job roda e reporta (nao e stub)
- [x] Zero regressao nos testes existentes (baseline mantida) — 51 backend fails são herdados de CRIT-001~006 (nenhuma regressão introduzida por CRIT-007, que é test-only)
