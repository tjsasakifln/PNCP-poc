...
- [ ] AC1: Backend (`portal_compras_client.py`): Mudar `valor=0.0` para `valor=None` (ou `valor_estimado=None`) para licitações PCP v2.
- [ ] AC2: Backend (`schemas.py`): Garantir que `valor_estimado` aceita `Optional[float]` (já é assim em muitos schemas, verificar BuscaResultItem).
- [ ] AC3: Frontend (`LicitacaoCard.tsx`): Quando `valor` for `null`/`undefined`/`0`, exibir "Valor não informado" em texto cinza (`text-ink-muted`) com tooltip "Fonte PCP v2 não disponibiliza valor estimado" em vez de "R$ 0".
- [ ] AC4: Frontend (`LicitacaoCard.tsx`): Remover função local `formatCurrency()` e substituir por import de `formatCurrencyBR` de `lib/format-currency.ts`.
- [ ] AC5: Verificar e corrigir todos os usos de `formatCurrency` local em outros componentes (LicitacoesPreview, etc.) para usar `formatCurrencyBR`.
- [ ] AC6: "R$ 0" nunca deve aparecer para o usuário final em nenhum componente.
- [ ] AC7: Valores acima de R$ 1 milhão devem usar abreviação ("R$ 3,5 mi") consistentemente em cards, pipeline e histórico.
...
## Testes Necessários
- [ ] Backend: Teste que licitação PCP retorna `valor=None` (não `0.0`).
- [ ] Frontend: Teste que `valor=null` renderiza "Valor não informado".
- [ ] Frontend: Teste que `valor=0` renderiza "Valor não informado" (não "R$ 0").
- [ ] Frontend: Teste que `valor=3500000` renderiza "R$ 3,5 mi".
- [ ] Frontend: Teste que `valor=45000` renderiza "R$ 45.000".
- [ ] Frontend: Snapshot visual do card com valor null vs valor positivo.
