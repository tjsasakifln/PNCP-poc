...
4. **Owner cutover.** Run `python3 -m bridge.preflight`. Only after that command is not `BLOCKED`, and only after a named host (`$BRIDGE_PUBLIC_IPV4`) and ACME email exist. Commands live in `SmartLic/bridge/docs/CUTOVER.md` and are also printed by the preflight. This goal does not run them.
