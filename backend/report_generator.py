...
class ReportGenerator:
    ...
    def generate_report(
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"**Generated by:** *acha-leads workflow\n")
            f.write(
            ...
            if leads:
                ...
                # Top 3 by score
                top_leads = sorted(
                    leads, key=lambda l: l.qualification.overall_score, reverse=True
                )[:3]
                f.write(
                    f"1. **High Priority:** Leads #"
                    + ", #".join(
                        str(leads.index(l) + 1) for l in top_leads
                    )
                    + f" (scores: "
                    + ", ".join(f"{l.qualification.overall_score:.1f}" for l in top_leads)
                    + ")\n"
                )
                f.write(
                    f"2. **Immediate Action:** Review leads with dependency ≥85%\n"
                )
                f.write(
            ...
            f.write(f"**Workflow Version:** 1.0.0\n")
            f.write(f"**Squad:** Lead Prospecting Task Force\n")
    ...
    def _write_lead_profile(self, f, index: int, lead: LeadProfile):
        ...
        f.write(
            f"## Lead #{index} - [NEW] {lead.company_name} ⭐ (Score: {lead.qualification.overall_score:.1f}/10)\n\n"
        )
        ...
        if lead.contact_data.whatsapp:
            f.write(f"- **WhatsApp:** {lead.contact_data.whatsapp} ✅\n")
        ...
        f.write(
            f"- **Dependency Score:** {lead.dependency_score.dependency_percentage:.1f}% 🎯 ({lead.dependency_score.dependency_level})\n"
        )
        f.write(
        ...
        f.write("```\n")
        sector_name = lead.sectors[0] if lead.sectors else "licitações públicas"
        f.write(f"Subject: Oportunidades de Licitação em {sector_name.title()} - SmartLic\n\n")
        ...
        f.write(f"- **Overall:** {q.overall_score:.1f}/10 ⭐\n\n")
