...
class TestHealthCanary:
    ...
    async def test_health_canary_failure_logs_warning(self, _mock_cron, caplog):
        ...
        assert any(
            "PNCP health" in record.message
            for record in caplog.records
        )
        assert any(
            "circuit breaker failure recorded" in record.message.lower()
            or "health check failed" in record.message.lower()
...
class TestBuscarComHealthCanary:
    ...
    @pytest.mark.asyncio
    async def test_canary_failure_returns_empty_result(self):
        """When canary fails, search returns empty ParallelFetchResult."""
        async with AsyncPNCPClient(max_concurrent=10) as client:
            async def mock_fetch_uf(*args, **kwargs):
                return [_make_item("FOUND-1")], False

            with patch.object(client, "health_canary", new_callable=AsyncMock, return_value=False), \
        ...
        assert len(result.items) == 0  # Canary failed → returns empty
