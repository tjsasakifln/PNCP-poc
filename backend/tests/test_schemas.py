...
class TestModalidadeContratacaoEnum:
    """Tests for ModalidadeContratacao enum (PNCP API codes — STORY-241)."""
...
class TestBuscaRequestModalidadeValidation:
    ...
    def test_excluded_modalidade_nine_rejected(self):
        ...
        assert "excluídas" in str(exc_info.value).lower() or "9" in str(exc_info.value)

    def test_excluded_modalidade_fourteen_rejected(self):
        ...
        assert "excluídas" in str(exc_info.value).lower() or "14" in str(exc_info.value)
