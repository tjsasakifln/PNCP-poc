...
class TestGetSessions:
    ...
    def test_single_session(self):
        ...
        sb = _mock_sb(data=[session_row], count=1)
        client = _create_client(mock_db=sb)
    ...
    def test_filters_by_authenticated_user_id(self):
        sb = _mock_sb(data=[], count=0)
        client = _create_client(mock_db=sb)
    ...
    def test_multiple_sessions_with_total(self):
        sessions = [
            {"id": f"sess-{i}", "user_id": "user-123-uuid", "created_at": f"2026-02-0{i}T10:00:00+00:00"}
            for i in range(1, 4)
        ]
        sb = _mock_sb(data=sessions, count=25)
        client = _create_client(mock_db=sb)
