...
class TestOpenAIClientTimeout:
    ...
    def test_timeout_configurable_via_env(self):
        ...
        CIG-BE-llm-arbiter-internals: llm_arbiter became a package and reads
        LLM_TIMEOUT_S indirectly via ``config.features``. Reload the chain so
        the env override is picked up: features → classification → facade.
        """
        import llm_arbiter
        import llm_arbiter.classification
        import config.features
        with patch.dict("os.environ", {"LLM_TIMEOUT_S": "30", "OPENAI_API_KEY": "test-key"}):
            import importlib
            importlib.reload(config.features)
            importlib.reload(llm_arbiter.classification)
