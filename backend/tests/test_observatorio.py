...
class TestObservatorioEndpoint:
    ...
    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_get_relatorio_200_no_auth(self, mock_query, client):
        """AC endpoint retorna 200 sem autenticação."""
        mock_query.return_value = MOCK_DATALAKE_RESULTS
        resp = client.get("/v1/observatorio/relatorio/3/2026")
        assert resp.status_code == 200

    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_get_relatorio_structure(self, mock_query, client):
    ...
    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_top_ufs_sorted_desc(self, mock_query, client):
    ...
    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_modalidade_distribution(self, mock_query, client):
    ...
    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_pct_sum_approximately_100(self, mock_query, client):
    ...
    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_empty_datalake_returns_zeros(self, mock_query, client):
    ...
    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_csv_endpoint_200(self, mock_query, client):
    ...
    @patch("datalake_query.query_datalake", new_callable=AsyncMock)
    def test_csv_has_source_attribution(self, mock_query, client):
