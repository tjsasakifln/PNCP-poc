...
## Caso de Referência: Jasmin Alic Anti-Pattern (2026-03-05)

**Contexto**: Análise comparativa identificou 9 gaps entre agente jasmin-alic (instagram-spy) e padrões do squad content-engine. Recomendação: "alinhar todos os 9 gaps".

**Resultado**: 5.5 de 9 ajustes eram bloat. Revertidos após revisão crítica.

**Falha na defensibilidade**: A evidência era "CE tem X e Jasmin não" — isto é N=1 (um squad), não padrão observado (N>=3). Pela RULE-RD-001, deveria ter sido MONITORAR, não ação.

**Anti-patterns detectados**:

| Anti-pattern | Exemplo | Regra violada |
|-------------|---------|---------------|
| Spec morta contada como feature | `memory_protocol` referenciando arquivo inexistente | Evidência > presença |
| Alinhamento cosmético | Renomear `quality_gate` → `self_check` sem mudança funcional | ROI = 0 |
| YAML onde texto funciona melhor | `REQUEST-RESOLUTION` reestruturado para YAML | Contexto importa (LLMs ≠ parsers) |
| Score inflado | 13→16/18 com specs mortas | Honestidade > métrica |
| Duplicação | `output_format` duplicando exemplos existentes | DRY |

**Regra derivada**: "Alinhar com padrão X" SÓ é recomendação válida se a AUSÊNCIA do padrão causa problema demonstrável (N>=3 ocorrências de falha).
